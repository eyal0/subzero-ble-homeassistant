...
3. **Print** `git push` and `gh pr create` for the user to run in their own terminal. Do not run them.
...
Give the user the exact commands (this clone’s remote is `eyal0`; `push.autosetupremote=true` so `git push` needs no `-u`):
