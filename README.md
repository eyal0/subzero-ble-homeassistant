...
**Show PIN** asks the appliance to show the 6-digit code on its display for 20 seconds. That write uses the encrypted control channel, so a PIN must already be saved in **Configure**. Open the appliance door if the code is not visible. It cannot bootstrap the first PIN by itself.
...
| Show PIN | Shows the 6-digit PIN on the appliance display for 20 seconds (needs a configured PIN and a bonded link). |
